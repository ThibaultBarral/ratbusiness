...
function ArticleImage({ url }: { url?: string }) {
    // Affichage d'image d'article avec signedUrl, lazy loading pour performance mobile
    ...
    return <img src={signedUrl} loading="lazy" alt="Article" className="w-20 h-20 object-cover rounded" />;
}
        ...
        const { error } = await supabase.from("sales").insert([
            {
                ...
                sale_date: saleDate,
                user_id: user.id, // ➔ on ajoute l'user_id ici
            },
        ]);
